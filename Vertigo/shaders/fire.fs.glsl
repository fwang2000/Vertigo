...
// Colour Gradient
#define color1 vec4(vec3(0.375), 1.0)
#define color2 vec4(0.425, 0.051, 1.0, 1.0)
#define color3 vec4(0.112, 0.0, 1.0, 1.0)
...
{
	float noise = Turbulence(position * NoiseFrequency + Animation* index, 0.1, 1.5, 0.03) * NoiseAmplitude;
...
void main()
{
	...
	float zoom = 5.0;
	// camera
	vec3 ro = zoom * normalize(vec3(cos(0.01), cos(0.01), sin(0.0)));
	vec3 ww = normalize(vec3(0.0, 0.0, 0.0) - ro);
	vec3 uu = normalize(cross( vec3(0.0, 1.0, 0.0), ww));
	vec3 vv = normalize(cross(ww, uu));
	vec3 rd = normalize(vpos.x*uu + vpos.y*vv + 1.5*ww);
	vec4 col = background;
	vec3 origin;
	...
	color = col * vec4(fcolor * vcolor, 1.0);
}